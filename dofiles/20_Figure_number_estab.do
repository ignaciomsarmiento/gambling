...
gen h_low =(VGTcount_within2<4 & within2==1)
gen h_midlow =(VGTcount_within2>=4 & VGTcount_within2<7) & within2==1
gen h_midhigh =(VGTcount_within2>=7 & VGTcount_within2<14) & within2==1
gen h_high =(VGTcount_within2>=15) & within2==1
...
local k=`k'+1
    ...
    twoway scatter B2 n, msymbol(+) mcolor(black)  || rcap B1 B3 n, lcolor(black) xlabel(1 "1/3" 2 "4/6" 3 "7/14" 4 "15/more", axis(1) angle(90)) yline(0, lcolor(gray)) yscale(range(-0.20(.1)0.20)) ylabel(-0.20(0.1)0.20)  legend(label(1 "Coefficient") label(2 "90% CI") position(4) ring(0) region(lcolor(none) fcolor(none))) xtitle("Number of Establishments with Video Gambling Within 2 Blocks") ytitle("Coefficient") ylabel(,nogrid) graphregion(color(white)) plotregion(lstyle(solid))
    graph export "${path_figures}/`depvar'_nrestab.eps", as(eps) replace
    restore
