...
destring police_* tract, replace

foreach depvar in ihs_violent ihs_property {

	
        * Baseline
    qui reghdfe `depvar'  within2   $ACS mindistrb mindistrb2 , absorb(id time i.comarea#i.time) vce(cl id)
    ...
    *Sides
    qui reghdfe `depvar'  within2   $ACS mindistrb mindistrb2 , absorb(id time i.sidecode#i.time) vce(cl id)
    estadd local rivboat      = "Yes", replace 
    estadd local demog        = "Yes", replace 
    estadd local neigh_trends = "Yes", replace 
    estadd local obs = e(N)
    ...
    *tract
    qui reghdfe `depvar'  within2   $ACS mindistrb mindistrb2 , absorb(id time i.tract#i.time) vce(cl id)
    estadd local rivboat      = "Yes", replace 
    estadd local demog        = "Yes", replace 
    estadd local neigh_trends = "Yes", replace 
    estadd local obs = e(N)
    ...
    *police_district
    qui reghdfe `depvar'  within2   $ACS mindistrb mindistrb2 , absorb(id time i.police_district#i.time) vce(cl id)
    estadd local rivboat      = "Yes", replace 
    estadd local demog        = "Yes", replace 
    estadd local neigh_trends = "Yes", replace 
    estadd local obs = e(N)
    ...
    *police_beat
    qui reghdfe `depvar'  within2   $ACS mindistrb mindistrb2 , absorb(id time i.police_beat#i.time) vce(cl id)
    estadd local rivboat      = "Yes", replace 
    estadd local demog        = "Yes", replace 
    estadd local neigh_trends = "Yes", replace 
    estadd local obs = e(N)
...
esttab model1_ihs_violent
        ...
        model4_ihs_violent
        model5_ihs_violent
       using "${path_tables}/diff_trends_by.tex",
       style(tex) 
